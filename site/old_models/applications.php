	...
	protected function prepareQuery(&$query)
	{
		...
		switch($this->getState('context', 'all'))
		{
			case 'layout.default':
				...
				break;
            case 'layout.evaluator':
                //SELECT : raw complete query without joins
                $this->addSelect('a.*');

                //SELECT
                $this->addSelect('jobs.short_description AS _job_id_short_description');
				$this->addSelect('status.description  AS _status_id_description');
                //$this->addSelect('_service_.name AS `_service_name`');

                //JOIN
                $this->addJoin('`#__recruitment_jobs` AS jobs ON jobs.id = a.job_id', 'LEFT');
                $this->addJoin('`#__recruitment_status` AS status ON status.id = a.status_id', 'LEFT');
				$this->addJoin('`#__recruitment_application_evaluator` AS evaluator ON evaluator.application_id = a.id', 'LEFT');
				$this->addJoin('`#__users` AS users ON users.id = evaluator.user_id', 'LEFT');
                /*$this->addJoin('`#__facturacioserveis_icmabpeople` AS _icmab_ip_ ON _icmab_ip_.id = a.icmab_ip', 'LEFT');
                $this->addJoin('`#__facturacioserveis_customerpeople` AS _responsable_person_ ON _responsable_person_.id = a.responsable_person', 'LEFT');
                $this->addJoin('`#__facturacioserveis_servicestatus` AS _service_status_ ON _service_status_.id = a.service_status', 'LEFT');
                $this->addJoin('`#__users` AS _users_1_ ON _users_1_.id = a.created_by', 'LEFT');

                $this->addWhere("a.internal = '1'");
                $this->setState('list.limit', 1000);
                $this->setState('list.start', 0);
                $query->order('a.id desc');*/

                $user = JFactory::getUser();
				$this->addWhere("evaluator.user_id = '$user->id'");
