	...
	protected function loadFormData()
	{
		...
		if (empty($data)) {
			...
			if ($this->getState('application.id') == 0)
			{
                ...
                $data->eligibility1 = null;
                $data->eligibility2 = null;
	...
	protected function prepareQuery(&$query, $pk)
	{
		...
		switch($this->getState('context', 'all'))
		{
			...
			case 'all':
				...
				//SELECT
				$this->addSelect('wheredidu.description AS _wheredidu');
				$this->addSelect('country.printable_name AS _birth_country');
				$this->addSelect('gender.description AS _gender');

				//JOIN
				$this->addJoin('`#__recruitment_wheredidus` AS wheredidu ON wheredidu.id = a.wheredidu_id', 'LEFT');
				$this->addJoin('`#__recruitment_countries` AS country ON country.id = a.birth_country_id', 'LEFT');
				$this->addJoin('`#__recruitment_genders` AS gender ON gender.id = a.gender_id', 'LEFT');
