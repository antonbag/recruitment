...
class RecruitmentCkHelperForm
{
	...
	*/
	public static function getElementGroups($element)
	{
		...
		$model = RecruitmentHelper::componentModel((string)$element['model'], false);
		...
		foreach ($element->children() as $option)
		{
			...
			if ($model)
			{
				...
				$group->model = (strpos($relation->model, '.')?'':'recruitment.') . $relation->model;

				$model = RecruitmentHelper::componentModel($relation->model, false);
			}
			...
			// Label Key
			$group->alias = RecruitmentClassModelOrm::tableFieldAlias($group->on, false);
	...
	*/
	public static function getModel($field, $item = false)
	{
		...
		return RecruitmentHelper::componentModel($model, $item);
	}
...
if (!class_exists('RecruitmentHelperForm')){ class RecruitmentHelperForm extends RecruitmentCkHelperForm{} }
