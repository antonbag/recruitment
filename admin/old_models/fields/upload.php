...
* @package	Recruitment
*/
class RecruitmentCkFormFieldUpload extends JFormField
{
	/**
	...
	*/
	protected function getInputMaxsize()
	{
		...
		$maxsize = RecruitmentHelperFile::getUploadMaxSize(true, $uploadMaxsize);
	...
	*/
	protected function getInputThumb()
	{
		...
		if (!empty($this->value))
		{
			...
			$previewUrlSrc = RecruitmentHelperFile::getFileUrlFromElement($this->form, $this->element, $this->value);
	...
	*/
	protected function uploadActionsList()
	{
		...
		$list[] = array('value' => '', 'text' => JText::_("RECRUITMENT_UPLOAD_REMOVE_KEEP"), 'icon' => 'cancel');

		if (in_array('remove', $actions))
			$list[] = array('value' => 'remove', 'text' => JText::_("RECRUITMENT_UPLOAD_REMOVE_EJECT"), 'icon' => 'out');

		if (in_array('thumbs', $actions))
			$list[] = array('value' => 'thumbs', 'text' => JText::_("RECRUITMENT_UPLOAD_REMOVE_THUMBS_ONLY"), 'icon' => 'pictures');

		if (in_array('trash', $actions))
			$list[] = array('value' => 'trash', 'text' => JText::_("RECRUITMENT_UPLOAD_REMOVE_TRASH"), 'icon' => 'trash');

		if (in_array('delete', $actions))
			$list[] = array('value' => 'delete', 'text' => JText::_("RECRUITMENT_UPLOAD_REMOVE_DELETE"), 'icon' => 'remove');
...
if (!class_exists('JFormFieldUpload')){ class JFormFieldUpload extends RecruitmentCkFormFieldUpload{} }
