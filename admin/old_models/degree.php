...
* @subpackage	Classes
*/
class RecruitmentCkModelDegree extends RecruitmentClassModelItem
{
	/**
	* View list alias
	*
	...
	*/
	public function getTable($type = 'degree', $prefix = 'RecruitmentTable', $config = array())
	{
	...
	*/
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_recruitment.edit.degree.data', array());
		...
		if (empty($data)) {
			...
			if ($this->getState('degree.id') == 0)
			{
				...
				$data->overall_range_id = $jinput->get('filter_overall_range_id', $this->getState('filter.overall_range_id'), 'INT');
	...
	* @param	integer	$pk	The primary id key of the degree
	*
	* @return	void
	*/
	...
	{
		//FROM : Main table
		$query->from('#__recruitment_degrees AS a');
	...
	*/
	public function save($data)
	{
		...
		$data['start_date'] = RecruitmentHelperDates::getSqlDate($data['start_date'], array('Y-m-d'), true, 'USER_UTC');

		//Convert from a non-SQL formated date (end_date)
		$data['end_date'] = RecruitmentHelperDates::getSqlDate($data['end_date'], array('Y-m-d'), true, 'USER_UTC');
...
if (!class_exists('RecruitmentModelDegree')){ class RecruitmentModelDegree extends RecruitmentCkModelDegree{} }
