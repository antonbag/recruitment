...
* @subpackage	Classes
*/
class RecruitmentCkModelDoc extends RecruitmentClassModelItem
{
	/**
	* View list alias
	*
	...
	*/
	public function getTable($type = 'doc', $prefix = 'RecruitmentTable', $config = array())
	{
	...
	*/
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_recruitment.edit.doc.data', array());
		...
		if (empty($data)) {
			...
			if ($this->getState('doc.id') == 0)
			{
				...
				$data->id = 0;
				$data->doc_type_id = $jinput->get('filter_doc_type_id', $this->getState('filter.doc_type_id'), 'INT');
	...
	* @param	integer	$pk	The primary id key of the doc
	*
	* @return	void
	*/
	...
	{
		//FROM : Main table
		$query->from('#__recruitment_docs AS a');
...
if (!class_exists('RecruitmentModelDoc')){ class RecruitmentModelDoc extends RecruitmentCkModelDoc{} }
