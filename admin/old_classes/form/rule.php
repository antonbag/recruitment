...
* @subpackage	Form
*/
class RecruitmentCkClassFormRule extends JFormRule
{
	/**
	...
	*/
	public static function testDefaults(&$element, $value, $group = null, &$input = null, &$form = null)
	{
		...
		if ($unique)
		{
			...
			if ($duplicate)
			{
				JError::raiseWarning(1201, JText::sprintf("RECRUITMENT_MODEL_DUPLICATE_ENTRY_FOR", JText::_($element['label'])));
				return false;
			}
		}
		...
		if (!empty($uniqueGroups))
		{
			...
			foreach($uniqueGroups as $alias => $uniqueGroup)
			{
				...
				if ($duplicate)
				{
					JError::raiseWarning(1201, JText::sprintf("RECRUITMENT_MODEL_DUPLICATE_ENTRY_FOR", implode (', ', $fieldLabels)));
					return false;
				}
			}
		}
...
if (!class_exists('RecruitmentClassFormRule')){ class RecruitmentClassFormRule extends RecruitmentCkClassFormRule{} }
