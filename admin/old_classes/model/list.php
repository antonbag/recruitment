...
* @subpackage	Classes
*/
class RecruitmentCkClassModelList extends JModelList
{
	/**
	* Data array
	*
	...
	*/
	...
	{
		$model = CkJModel::getInstance($this->view_item, 'RecruitmentModel');
	...
	*/
	public function getItems()
	{
		try
		{
			...
			{
				if (!$this->canAdmin())
					$error = JText::_('RECRUITMENT_ERROR_INVALID_QUERY');
	...
	*/
	...
	{
		if ($item)
			return CkJModel::getInstance($this->view_item, 'RecruitmentModel');
	...
	*/
	...
	{
		$url = 'index.php?option=com_recruitment';
	...
	*/
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$globalParams = JComponentHelper::getParams('com_recruitment', true);
		...
		// FILTERS
			foreach($this->filter_vars as $var => $varType)
			{
				...
				{
					$date = RecruitmentHelperDates::timeFromFormat($value, $matches[1]);
					if ($date)
					{
	...
	*/
	public function updateXref($field, $values, $on, $id)
	{
		...
		$model = CkJModel::getInstance($this->getName(), 'RecruitmentModel');
		...
		else
		{
			...
			{
				$query = $db->getQuery(true);
				$query->update('#__recruitment_' . $this->getName())
...
if (!class_exists('RecruitmentClassModelList')){ class RecruitmentClassModelList extends RecruitmentCkClassModelList{} }
