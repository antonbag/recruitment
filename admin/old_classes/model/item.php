...
* @subpackage	Classes
*/
class RecruitmentCkClassModelItem extends JModelAdmin
{
	/**
	* Data array
	*
	...
	*/
	public function __construct($config = array())
	{
		...
		// Load the ORM system
		$this->orm = RecruitmentClassModelOrm::getInstance($this);
	...
	*/
	public function canCheck($record)
	{
		...
		{			
			$this->setError(JText::_("RECRUITMENT_TASK_RESULT_THE_USER_CHECKING_OUT_DOES_NOT_MATCH_THE_USER_WHO_CHECKED_OUT_THE_ITEM"));
			return false;			
		}
	...
	*/
	public function deletefiles($pks, $fileFields)
	{
		...
		$config	= JComponentHelper::getParams( 'com_recruitment' );
		...
		foreach($fileFields as $fieldName => $op)
		{
			...
			foreach($files as $fileObj)
			{
				...
				if (!RecruitmentHelperFile::removeFile($imagePath, $op))
					$errors = true;
			}
		}
	...
	*/
	public static function fkTable($fieldname)
	{
		...
		switch($fieldname)
		{
			case 'tab_id': return $tbl.$com. 'tabs';
			case 'job_id': return $tbl.$com. 'jobs';
			case 'status_id': return $tbl.$com. 'status';
			case 'application_id': return $tbl.$com. 'applications';
			case 'overall_range_id': return $tbl.$com. 'overallranges';
			case 'country_id': return $tbl.$com. 'countries';
			case 'user_id': return $tbl. 'users';
			case 'gender_id': return $tbl.$com. 'genders';
			case 'birth_country_id': return $tbl.$com. 'countries';
			case 'wheredidu_id': return $tbl.$com. 'wheredidus';
			case 'doc_type_id': return $tbl.$com. 'doctypes';
			case 'programme_id': return $tbl.$com. 'programmes';	
		}
	}
	...
	*/
	...
	{
		$model = CkJModel::getInstance(ucfirst($table), 'RecruitmentModel');
	...
	*/
	public function getForm($data = array(), $loadData = true, $control = 'jform')
	{
		...
		$form->addRulePath(JPATH_ADMIN_RECRUITMENT . '/models/rules');
	...
	*/
	...
	{
		$modelList = CkJModel::getInstance($this->getNameList(), 'RecruitmentModel');
		if ($modelList)
	...
	*/
	public function integrityRestrict($relation, &$pks, $labelKey, $parentLabelKey, $editLink)
	{
		...
		foreach($pks as $id)
		{
			...
			$children = RecruitmentHelper::getData($childModel, array(
				'filter' => array(
			...
			if (count($children))
			{
				...
				$model = RecruitmentHelper::componentModel($relation->foreignModelClass, true);
				...
				$msg = sprintf(JText::_('RECRUITMENT_ERROR_IMPOSSIBLE_TO_DELETE_USED_IN'), ($itemName?$itemName:'item'), implode(', ', $childText));
	...
	*/
	protected function loadForm($name, $source = null, $options = array(), $clear = false, $xpath = false)
	{
		...
		// Get the form.
		JForm::addFormPath(JPATH_RECRUITMENT . '/models/forms');
		JForm::addFieldPath(JPATH_RECRUITMENT . '/models/fields');
	...
	*/
	public function loadRelation($name, &$item, $orm = array())
	{
		...
		$model = RecruitmentClassModelOrm::getModel($relation->foreignModelClass);
		if (!$model)
			return;
	...
	*/
	public function loadXref(&$item, $objectField, $xrefTable, $on, $key, $states = array(), $context = 'object.default')
	{
		...
		{
			$model = CkJModel::getInstance($xrefTable, 'recruitmentModel');
	...
	*/
	protected function populateState()
	{
		...
		if (defined('JDEBUG'))
			$_SESSION["Recruitment"]["Model"][$this->getName()]["State"] = $this->state;
	...
	*/
	...
	{
		$baseFolder = JPATH_RECRUITMENT . '/fork/models/forms';
	...
	*/
	...
	{
		return RecruitmentClassModelOrm::queryDump($query, $output);
	}
	...
	*/
	public function toggle($fieldName, $pk = null, $max = 1)
	{
		...
		if (!$result)
		{
			JError::raiseWarning(1106, JText::sprintf("RECRUITMENT_MODEL_IMPOSSIBLE_TO_TOGGLE", $fieldName));
			return false;
		}
			...
			switch($field->type)
			{
				...
				case 'ckcalendar':
					...
					if ($value && (string)$field->format)
					{
						...
						if (!is_numeric($value) && !RecruitmentHelperDates::isNull((string)$value))
						{
							$time = RecruitmentHelperDates::getSqlDate($value, array($field->format));

							if ($time === null){
								JError::raiseWarning(1203, JText::sprintf('RECRUITMENT_VALIDATOR_WRONG_DATETIME_FORMAT_FOR_PLEASE_RETRY', $field->label));
								$valid = false;
							}
							else
...
if (!class_exists('RecruitmentClassModelItem')){ class RecruitmentClassModelItem extends RecruitmentCkClassModelItem{} }
