...
* @subpackage	
*/
class RecruitmentCkClassControllerItem extends JControllerForm
{
	/**
	...
	* @var string
	*/
	protected $text_prefix = 'COM_RECRUITMENT';
	...
	*/
	public function add()
	{
		...
		$this->applyRedirection($result, array(
			'com_recruitment.' . $this->view_list . '.default',
			'com_recruitment.' . $this->view_item . '.' . $this->getLayout('edit')
		));
	}
	...
	*/
	protected function applyRedirection($result, $redirections, $vars = array())
	{
		...
		switch($redirection)
		{
			...
			case 'stay':
				$this->setRedirect(RecruitmentHelper::urlRequest($vars));
				return;
				break;
		...
		//Filters followers
		$model = CkJModel::getInstance($this->view_list, 'RecruitmentModel');
		if ($model)
		{
	...
	*/
	public function delete()
	{
		...
		else
		{
			...
			{
				$result = true;
				$this->setMessage(JText::plural('RECRUITMENT_ITEMS_SUCCESSFULLY_DELETED', count($cid)));
			}
			else
			{
	...
	public function redirect()
	{
		if ($this->redirect)
		{
			...
			if ($jinput->get('return') == 'json')
			{
				...
				RecruitmentClassAjax::responseJson(array(
	...
	*/
	protected function updateFileField($model, $fieldName, $options = array())
	{
		...
		try
		{
			$changedValue = RecruitmentHelperFile::updateFile($model, $fieldName, $options, $uploadDirectory);
		}
		catch(Exception $e)
		{
...
if (!class_exists('RecruitmentClassControllerItem')){ class RecruitmentClassControllerItem extends RecruitmentCkClassControllerItem{} }
